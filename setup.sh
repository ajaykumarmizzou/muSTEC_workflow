...
merged=$("./results/variant_calling/merged_variants/merged.vcf" | sed 's|/data/tura_data||')
...
#Here each sample for two paired end files reads and continues for the pipeline
    ...
    total_files=$(ls -l data/tura_SRR_samples/fastq_files/*.fastq | wc -l)
    echo "Total files: $total_files"
    ...
    do
        echo "Sample $((i/2+1)):"
        file_1=$(ls data/tura_SRR_samples/fastq_files/*.fastq | sed -n "$i"p)
        # less $file_1
        echo "Read 1: $file_1"
        file_2=$(ls data/tura_SRR_samples/fastq_files/*.fastq | sed -n "$j"p)
        # less $file_2
        echo "Read 2: $file_2"
        # echo "Is this correct? (y/n)"
        # read ans
...
#Trimming and quality control
        perl ./tools/trim-galore/trim_galore.pl --paired --fastqc --illumina --output_dir results/tura_results/trimmed/ $file_1 $file_2
#indexing reference genome
        # bwa index $ref
#Alignment of sequences samples to reference genome - samtools alignment
#run bwa mem for each sample
        f1=$(echo "./results/tura_results/trimmed/"${file_1%%.*}_val_1.fq"" | sed 's|data/tura_SRR_samples/fastq_files/||')
        f2=$(echo "./results/tura_results/trimmed/"${file_2%%.*}_val_2.fq"" | sed 's|data/tura_SRR_samples/fastq_files/||')
        s=$(echo "./results/tura_results/aligned/sam/"${file_1%%.*}.sam"" | sed 's|data/tura_SRR_samples/fastq_files/||')
        # echo $s
        bwa mem $ref $f1 $f2 > $s
        echo "Alignment done!"
#Converting sam file into bam
        bam=$(echo "./results/tura_results/aligned/bam/"${file_1%%.*}.bam"" | sed 's|data/tura_SRR_samples/fastq_files/||')
        samtools view -bS $s > $bam
        echo "Converting sam into bam, done!"
#Sorting bam file
        sorted=$(echo "./results/tura_results/aligned/sorted_bam/"${file_1%%.*}.sorted.bam"" | sed 's|data/tura_SRR_samples/fastq_files/||')
        samtools sort $bam -o $sorted
        echo "Sorting bam, done!"
#samtools flagstats
        flagstats=$(echo "./results/tura_results/aligned/flagstats/"${file_1%%.*}.flagstats.txt"" | sed 's|data/tura_SRR_samples/fastq_files/||')
        samtools flagstat $sorted > $flagstats
        echo "Samtools flagstats, done!"
#bcf mpileup
        mpileup=$(echo "./results/tura_results/variant_calling/bcf/"${file_1%%.*}.bcf"" | sed 's|data/tura_SRR_samples/fastq_files/||')
        bcftools mpileup -O b -o $mpileup -f $ref $sorted
        echo "bcf mpileup, done!"
#Identify SNVs using call ploidy
        vcf=$(echo "./results/tura_results/variant_calling/vcf/"${file_1%%.*}.vcf"" | sed 's|data/tura_SRR_samples/fastq_files/||')
        bcftools call --ploidy 1 -m -v -o $vcf $mpileup
#         echo "Identifying SNVs, done!"
#Filter using varFilter
        filtered=$(echo "./results/tura_results/variant_calling/filtered_vcf/"${file_1%%.*}.vcf"" | sed 's|data/tura_SRR_samples/fastq_files/||')
#         vcfutils.pl varFilter $vcf > $filtered
#         echo "Filtering SNVs, done!"
#bcftools stats to generate vcf stats
        stats=$(echo "./results/tura_results/variant_calling/vcf_stats/"${file_1%%.*}.vcf.txt"" | sed 's|data/tura_SRR_samples/fastq_files/||')
        bcftools stats -F $ref -s - $filtered > $stats
        echo "Generating vcf stats, done!"
#plot-vcfstats
        ...
        # echo "Plotting vcf stats, done!"
#sort variants
        sorted_vcf=$(echo "./results/tura_results/variant_calling/sorted_vcf/"${file_1%%.*}.sorted.vcf"" | sed 's|data/tura_SRR_samples/fastq_files/||')
        bcftools sort -o $sorted_vcf $filtered
        echo "Sorting vcf, done!"
#bgzip and tabix
        bgzip $sorted_vcf
...
#Merge into single vcf file
    bcftools merge ./results/tura_results/variant_calling/sorted_vcf/*.gz > ./results/tura_results/variant_calling/merged_variants/merged.vcf
    echo "Merging vcf, done!"
#Annotating multiVCF file using snpEff
    merged=$("./results/tura_results/variant_calling/merged_variants/merged.vcf" | sed 's|/data/tura_data||')
    java -Xmx8g -jar /data/akt5b/.Github_repositories/muSTEC_workflow/muSTEC_workflow/tools/snpEff/snpEff/snpEff.jar  -v e.coli.o157.h7.sakai  $merged > test.chr22.ann.vcf
#multiqc
# multiqc ./results/tura_results/trimmed/ -o ./results/tura_results/multiqc_trimmed/
